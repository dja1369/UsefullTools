...
async def package_images(target_date: dict):
    img_group = get_image_group_by_date(target_date, orm.get_package_data_by_created_at_range)
    await download_images(img_group)

async def sample_images(target_date: dict):
    img_group = get_image_group_by_date(target_date, orm.get_sample_data_by_created_at_range)
    await download_images(img_group)

def get_image_group_by_date(target_date: dict, data_fetch_func: callable):
    img_group = {}
...
async def download_images(img_group: dict):
    coroutine_arr = []
...
def find_missing_sample():
    target_date = date_util.search_all_date(datetime(2024, 1, 1), datetime(2024, 9, 10))
    img_group = get_image_group_by_date(target_date, orm.get_all_sample_date_by_issue_tag_match)
    merge_img_and_tag_group = merge_images_and_tags(img_group)
    merge_rotate_group = merge_rotations(merge_img_and_tag_group)
    check_files_existence(merge_rotate_group)
    save_to_excel(merge_rotate_group)

def merge_images_and_tags(img_group):
    # 이슈 와 태그 정보 병합
    merge_img_and_tag_group = {}
    for k, v in img_group.items():
        if v:
            for obj in v:
                tag = orm.get_tag_by_tag_code(obj.tag_code)
                tag_info = get_tag_info(obj, tag)
                merge_img_and_tag_group.setdefault(k, []).append(tag_info)
    return merge_img_and_tag_group

def get_tag_info(obj, tag):
    if not tag:
...
def merge_rotations(merge_img_and_tag_group):
    # 이슈와 회전된 이슈 정보 병합
    merge_rotate_group = {}
    for k, v in merge_img_and_tag_group.items():
        for obj in v:
            temp = {obj[0]: {0: obj}}
            package_link_sample = orm.get_all_sample_date_by_package_link(obj[0])
...
def check_files_existence(merge_rotate_group):
    # 파일 존재 여부 확인
    remote_path = f"input"
        ...
        for issue in issue_arr:
            for k, v in issue.items():
                for rotate in [0,0,0]:
                    ...
                    for position in ["condition", "condition"]:
                        v[rotate].append(client.is_exist(f"input"))

def save_to_excel(merge_rotate_group):
    # 엑셀로 저장
    result_group = {}
    count = 0
    for day, issue_arr in merge_rotate_group.items():
